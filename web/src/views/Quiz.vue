...
const saveProgress = () => {
  const progress = { bankId: bankId.value, currentIndex: currentIndex.value, questions: questions.value, correctCount: correctCount.value, totalCount: totalCount.value, timestamp: Date.now() }
  localStorage.setItem('quizProgress', JSON.stringify(progress))
}
const loadProgress = () => { const saved = localStorage.getItem('quizProgress'); if (!saved) return null; try { return JSON.parse(saved) } catch { return null } }
const clearProgress = () => { localStorage.removeItem('quizProgress') }
...
const loadQuestions = async () => {
  ...
  try {
    ...
    if (saved && saved.bankId === bankId.value && saved.questions && saved.questions.length > 0) {
      questions.value = saved.questions; currentIndex.value = saved.currentIndex; correctCount.value = saved.correctCount; totalCount.value = saved.totalCount; hasUnfinished.value = true
      ElMessage.success('已恢复上次进度')
    } else {
      const data = await quizApi.start(bankId.value, quizCount.value)
      if (!data || data.length === 0) { ElMessage.warning('题库为空'); loading.value = false; return }
      questions.value = data; currentIndex.value = 0; correctCount.value = 0; totalCount.value = 0; hasUnfinished.value = false; clearProgress()
    }
    questionState.value = 'answering'; userAnswer.value = ''; feedback.value = null; newAnswer.value = ''; newAnalysis.value = ''; aiExplanation.value = ''; loadingAI.value = false; showResult.value = false
    updateCurrent()
...
const updateCurrent = () => {
  ...
  currentOptions.value = options.map(opt => { const val = opt.slice(0, 1); const selected = q.type === 'multi' ? (userAnswer.value || '').includes(val) : userAnswer.value === val; return { text: opt, val, selected } })
...
const selectAnswer = (val) => {
  ...
  if (q.type === 'multi') { let cur = userAnswer.value || ''; cur = cur.includes(val) ? cur.replace(val, '') : cur + val; userAnswer.value = cur.split('').sort().join('') }
  else { userAnswer.value = val }
  updateCurrent()
}
...
const confirmAnswer = async () => {
  ...
  try {
    ...
    feedback.value = { isCorrect: r.is_correct, myAnswer: ans, correctAnswer: r.answer, analysis: r.analysis, hasAnswer: r.has_answer }
    questionState.value = 'confirmed'
    if (r.is_correct === true) correctCount.value++
    totalCount.value++; saveProgress()
    getAIExplanation()
...
const submitNewAnswer = async () => {
  if (!newAnswer.value.trim()) { ElMessage.warning('请输入答案'); return }
  try {
    const q = currentQuestion.value
...
const saveAndExit = () => { saveProgress(); ElMessage.success('进度已保存') }
const continueQuiz = () => { hasUnfinished.value = false; showResult.value = false; updateCurrent() }
const goPrev = () => {
  if (currentIndex.value <= 0) return
  currentIndex.value--; questionState.value = 'answering'; userAnswer.value = ''; feedback.value = null; newAnswer.value = ''; newAnalysis.value = ''; aiExplanation.value = ''; loadingAI.value = false
  updateCurrent(); saveProgress()
}
const goNext = () => {
  if (currentIndex.value >= questions.value.length - 1) {
    ...
    showResult.value = true; clearProgress(); return
  }
  currentIndex.value++; questionState.value = 'answering'; userAnswer.value = ''; feedback.value = null; newAnswer.value = ''; newAnalysis.value = ''; aiExplanation.value = ''; loadingAI.value = false
  updateCurrent(); saveProgress()
}
const retry = () => { showResult.value = false; clearProgress(); loadQuestions() }
onBeforeUnmount(() => { if (!showResult.value && questions.value.length > 0) saveProgress() })
