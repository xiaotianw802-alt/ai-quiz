...
router.post("/submit", (req, res) => {
  try {
    ...
    const stmt = db.prepare("SELECT answer, analysis FROM questions WHERE id=?");
    ...
    for (const a of answers) {
      ...
      if (!q) continue;
      const isCorrect = q.answer && a.user_answer.trim().toUpperCase() === q.answer.trim().toUpperCase();
...
router.post("/explain", async (req, res) => {
  try {
    ...
    const messages = [
      { role: "system", content: "你是一个耐心的刷题辅导老师，请用通俗易懂的方式解释题目。如果题目有选项，请逐一分析每个选项为什么对或错。" },
